...
9. **Ошибки в самом процессе работы.** `pkill -f server/src/index.ts` совпал с командной строкой собственной оболочки ИИ и убил её, со следующей попыткой через `ps | grep` было то же самое. Перешли на pid-файл. Тест видимости фокуса падал из-за самого теста: после `blur()` Chrome не знает, откуда начинать Tab, а `console.log` внутри `page.evaluate` уходит в консоль браузера, а не в вывод теста. Стартовую точку теперь задаёт клик по заголовку, как сделал бы человек.
