...
браузер ──POST /api/chat──▶ наш сервер ──stream──▶ OpenRouter
        ◀── SSE: meta / thinking / delta / done / error ──┘
```
...
| модель молчит | сервер: 60 с до первого токена, 30 с тишины потом | «модель слишком долго молчит» |
| соединение умерло молча | клиент: 45 с без единого байта (сервер шлёт ping каждые 15 с) | то же, таймаут |
...
В `.env` задаётся список моделей. OpenRouter сам переходит к следующей, если первая отдала 429 или недоступна, поэтому бесплатный лимит бьёт реже.
...
- **Переключение модели посреди ошибки.** Запасные модели OpenRouter помогают, только пока ответ не начался. В живой проверке nemotron на длинном вопросе «думал» так долго, что OpenRouter сам прислал 504 внутри уже открытого потока. Можно повторять запрос на следующей модели, если ошибка пришла до первого видимого токена.
...
- CI на GitHub Actions: тесты и e2e на каждый PR.
...
7. **Ошибки в самом процессе работы.** `pkill -f server/src/index.ts` совпал с командной строкой собственной оболочки ИИ и убил её, со следующей попыткой через `ps | grep` было то же самое. Перешли на pid-файл. Тест видимости фокуса падал из-за самого теста: после `blur()` Chrome не знает, откуда начинать Tab, а `console.log` внутри `page.evaluate` уходит в консоль браузера, а не в вывод теста. Стартовую точку теперь задаёт клик по заголовку, как сделал бы человек.
